...
team runs it — and keeps a `.cmd` launcher in the parent folder pointing at
`Software\Aplicativo\Gerencie Carteira.exe` (the exe keeps its neutral name —
renaming breaks Electron 33 ASAR integrity). robocopy `/MIR` removes the
previous build, so there is never more than one.
...
- **Artefatos de build:** nomes neutros (sem versao embutida). O `.exe`
  principal do Electron NAO pode ser renomeado: Electron 33 quebra ASAR
  integrity (crash 0x80000003 no boot). A versao chega a UI via IPC
  `app:version` em runtime — nao precisa estar no disco.
