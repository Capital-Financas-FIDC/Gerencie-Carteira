...
# Publicar na rede — build local + espelha p/ A:\...\Software (a equipe roda de la)
./publicar.ps1                            # build-app.ps1 + robocopy /MIR + versao no .exe
# manual equivalente:
...
(launches the local build, for testing). `publicar.ps1` mirrors that build to
`A:\PUBLICA\GERENCIE CARTEIRA PÚBLICA\Software\Aplicativo\` — where the whole
team runs it — renames the `.exe` with the version (derived from `package.json`)
and keeps a wildcard `.cmd` launcher in the parent folder. robocopy `/MIR`
removes the previous build, so there is never more than one.
...
On every run the Python core idempotently bootstraps the work folders
`{planilhas,html,logs}` under a `data` root — `main()` derives that root from
`config.ini` (`[Paths]`) and passes it to `directory_bootstrap.ensure_workspace`.
With the app hosted on the share, the `data` root is
...
- **Artefatos de build:** scripts e arquivos-fonte tem nomes neutros. EXCECAO: o
  `.exe` distribuido por `publicar.ps1` carrega a versao no nome
  (`Gerencie Carteira <versao>.exe`), DERIVADA de `package.json` no momento da
  publicacao — nao e hardcoded, continua fonte unica.
...
Nao reintroduzir versao em nomes de arquivo-fonte, comentarios de config,
README, JSX ou strings do Python — todos derivam da fonte unica. O `.exe`
distribuido por `publicar.ps1` e a unica excecao (nome derivado, ver acima).
